# Demo Script (2 min — Showcase cut)

**0:00** Mark: one objective in the group — “Ship grok-bot-spec-to-ship kit.”  
...
## Kickoff (paste into the group)

```
@everyone Spec-to-ship live run. Objective: ship public repo grok-bot-spec-to-ship — Bot profiles, operating protocol, Notion+GitHub templates, examples/live-run/ from this run, and demo/SCRIPT.md. Orchestrator owns plan/assign. Researcher → one-pager evidence pack. Product Design → paste-ready profiles + PRD for the kit. Engineer → scaffold + PR. Reviewer → one pass on each artifact vs SoT. Room: assignment/handoff/blocker/verdict only. Mark: judgment only. Go.
```

## Presenter note
Lead with the live repo + room handoffs. See `examples/live-run/06-presenter-cues.md`.
